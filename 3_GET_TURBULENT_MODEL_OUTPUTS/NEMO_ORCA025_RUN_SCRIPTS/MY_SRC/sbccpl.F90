...
!##################################################
!2019-01-13 DPS: uncommented the below region due to model complaint of negative runoff
...
!##################################################
!2019-01-13 DPS uncommented due to model complaint of negative runoff
      zcumulpos = SUM( MAX( frcv(jpr_rnf)%z3(:,:,1), 0.e0 ) * e1t(:,:) * e2t(:,:) * tmask_i(:,:) ) 
      zcumulneg = SUM( MIN( frcv(jpr_rnf)%z3(:,:,1), 0.e0 ) * e1t(:,:) * e2t(:,:) * tmask_i(:,:) )
      ...
      IF( lk_mpp )   CALL mpp_sum( zcumulneg ) 
      IF(lwp) THEN                        ! control print
         WRITE(numout,*) 'Negative runoff values:'
